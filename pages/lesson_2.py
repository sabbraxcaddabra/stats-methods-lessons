...
def __(dataclass, np):
    ...
    class Penetration:
        ...
        def rs(self, t, y):
            ...
            sigmat_u: float = self.sigmat_u
            c_zv_u: float = self.c_zv_u
            Cu: float = self.Cu
            Cp: float = self.Cp
            A: float = self.A
            Dp: float = self.Dp
            ...
            S = 0.5 * Rg * (v / u - 1) * (1 - 1/gamma**2)
            Du = rho_u * S

            u_num = sigmat_u * (1 + (v - u) / c_zv_u) + Cu * (v - u) ** 2 - A - Cp * u ** 2
            u_denum = Dp + Du

            udot = u_num / u_denum

            udot = udot if udot < 0 else 0

            vdot = -sigmat_u * (1 + (v - u) / c_zv_u) / (rho_u * (L - S))

            vdot = vdot if vdot < 0 else 0

            Ldot = u - v
            xdot = u

            return [udot, vdot, Ldot, xdot]
    return (Penetration,)
...
def __(Penetration, get_u0, np, solve_ivp):
    def model(data: dict):
        ...
        A = 2 * (
            sigmat_p * np.log(gamma) + (2/3) * E_p * (
                (1/18)*np.pi*np.pi * (1 - lam_p) * (1 - 1/np.cbrt(gamma)) ** (0.62) + 0.5 * sigmat_p / (E_p*lam_p)
            )
        )
...
def __(
    ...
    for i in range(n_samples.value):
        base_data['da'] = da_data[i]
        base_data['la'] = la_data[i]
        base_data['sigmat_u'] = sigmat_u_data[i]
        base_data['rho_u'] = rho_u_data[i]
        bi = newton(
            lambda vc: model(update_and_return_dict(base_data, vc))["x"][-1] - 1e-3*h_data[i],
            x0=1300, rtol=1e-2
        )
        ballistic_impact[i] = bi
    return ballistic_impact, bi, i
